...
async function getSingleActivity(req, res, next) {
  try {
    let activityData = await Activity.findByPk(parseInt(req.params.id), {
...
async function getAllActivities(req, res, next) {
  try {
    let activityData = await Activity.findAll({ include: activityInclude });
    res.json(activityData);
  } catch (error) {
    console.error(error);
    res.status(500).end();
...
async function updateSingleActivity(req, res, next) {
  ...
  const activity = await Activity.findByPk(id, { include: activityInclude });
  res.json(activity);
}
